  ...
  var Hooks2 = {
    ...
    Stage: {
      mounted() {
        ...
        const setDragState = (on) => {
          container.classList.toggle("stage__viewport--drag-over", on);
        };
        const onDragEnter = (e) => {
        ...
        let boundCompare = null;
        const teardownCompare = () => {
          if (!boundCompare)
            return;
          const { btn, start, stop } = boundCompare;
        ...
        const bindCompare = () => {
          ...
          const btn = document.querySelector('[data-stage-action="compare"]');
          if (boundCompare && boundCompare.btn === btn && boundCompare.img === img)
            return;
          teardownCompare();
          if (!img || !btn)
            return;
          ...
          const start = (e) => {
            e.preventDefault();
            const original = btn.getAttribute("data-compare-src");
            if (!original || processedSrc !== null)
              return;
            processedSrc = img.getAttribute("src");
          ...
          boundCompare = { btn, img, start, stop };
        };
        bindCompare();
        this.bindCompare = bindCompare;
        this.unbindCompare = teardownCompare;
        const maxPath = "M4 9V4h5M20 15v5h-5M15 4h5v5M9 20H4v-5";
        const minPath = "M9 4v5H4M15 20v-5h5M20 9h-5V4M4 15h5v5";
        const syncFullscreen = () => {
          const btn = document.querySelector('[data-stage-action="fullscreen"]');
          if (!btn)
            return;
          const on = document.fullscreenElement === container;
          const path = btn.querySelector("svg path");
          if (path)
            path.setAttribute("d", on ? minPath : maxPath);
          btn.setAttribute("aria-pressed", on ? "true" : "false");
          btn.title = on ? "Exit fullscreen" : "Toggle fullscreen";
        };
        container.addEventListener("app:fullscreen", () => {
        ...
        this.destroyFullscreen = () => document.removeEventListener("fullscreenchange", syncFullscreen);
        this.syncFullscreen = syncFullscreen;
      },
      updated() {
        if (this.bindCompare)
          this.bindCompare();
      ...
      destroyed() {
        if (this.cleanupDrop)
          this.cleanupDrop();
        if (this.unbindCompare)
          this.unbindCompare();
        if (this.destroyFullscreen)
    ...
    FolderBrowse: {
      mounted() {
        ...
        this.liveInput = this.el.dataset.uploadRef ? document.getElementById(this.el.dataset.uploadRef) : null;
        this.input.addEventListener("change", (e) => {
          const files = Array.from(e.target.files || []);
          const images = files.filter((f) => this.isImageFile(f));
          const skipped = files.length - images.length;
          if (images.length > 0 && this.liveInput) {
            const dataTransfer = new DataTransfer();
            images.forEach((f) => dataTransfer.items.add(f));
            this.liveInput.files = dataTransfer.files;
            this.liveInput.dispatchEvent(new Event("input", { bubbles: true }));
          }
          this.pushEvent("folder_browsed", { added: images.length, skipped });
          this.input.value = "";
        });
      ...
      isImageFile(file) {
        if ((file.type || "").startsWith("image/"))
          return true;
        return /\.(jpe?g|png|gif|webp|bmp|tif|tiff|avif)$/i.test(file.name || "");
      }
    }
  };
